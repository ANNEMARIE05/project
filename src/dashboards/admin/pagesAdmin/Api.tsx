...
export default function Api() {
    const [rechercheApis, setRechercheApis] = useState('');
    const [apiSelectionnee, setApiSelectionnee] = useState<string | null>(null);
    const [afficherClesApi, setAfficherClesApi] = useState(false);
    const [cleCopiee, setCleCopiee] = useState<string | null>(null);
    const [afficherCle, setAfficherCle] = useState<{[key: string]: boolean}>({});
    const [afficherJournalUtilisation, setAfficherJournalUtilisation] = useState<string | null>(null);
    const [modifierPermissions, setModifierPermissions] = useState<{[key: string]: boolean}>({});
    const [afficherLogsApi, setAfficherLogsApi] = useState<string | null>(null);
    const [pageActuelle, setPageActuelle] = useState(1);
    const [elementsParPage, setElementsParPage] = useState(4);
    const [afficherModalSuppression, setAfficherModalSuppression] = useState(false);
    const [apiASupprimer, setApiASupprimer] = useState<ApiEndpoint | null>(null);
    ...
    const apis: ApiEndpoint[] = [
      ...
      const apisFiltrees = apis.filter(api =>
        api.nom.toLowerCase().includes(rechercheApis.toLowerCase()) ||
        api.statut.toLowerCase().includes(rechercheApis.toLowerCase())
      );

      // Pagination logic
      const indexDernierElement = pageActuelle * elementsParPage;
      const indexPremierElement = indexDernierElement - elementsParPage;
      const apisActuelles = apisFiltrees.slice(indexPremierElement, indexDernierElement);
      const pagesTotales = Math.ceil(apisFiltrees.length / elementsParPage);
      ...
      const regenererCle = (idApi: string, idCle: string) => {
        // Logique pour régénérer une clé API
        console.log(`Régénération de la clé ${idCle} pour l'API ${idApi}`);
      };

      const mettreAJourPermissions = (idApi: string, idCle: string, permissions: ApiPermission) => {
        // Logique pour mettre à jour les permissions
        console.log(`Mise à jour des permissions pour la clé ${idCle} de l'API ${idApi}:`, permissions);
        setModifierPermissions({...modifierPermissions, [idCle]: false});
      };

      const supprimerApi = (api: ApiEndpoint) => {
        setApiASupprimer(api);
        setAfficherModalSuppression(true);
      };

      const confirmerSuppressionApi = () => {
        if (apiASupprimer) {
          // Logique pour supprimer l'API
          console.log(`Suppression de l'API ${apiASupprimer.id} - ${apiASupprimer.nom}`);
          // Ici vous pouvez ajouter la logique de suppression réelle
          setAfficherModalSuppression(false);
          setApiASupprimer(null);
        }
      };

      const annulerSuppressionApi = () => {
        setAfficherModalSuppression(false);
        setApiASupprimer(null);
      };

      const obtenirCouleurMethode = (methode: string) => {
        switch (methode) {
      ...
      const obtenirCouleurStatut = (statut: number | string) => {
        if (typeof statut === 'number') {
          if (statut >= 200 && statut < 300) return 'text-green-600';
          if (statut >= 400 && statut < 500) return 'text-yellow-600';
          if (statut >= 500) return 'text-red-600';
      ...
      const formaterDateHeure = (chaineDate: string) => {
        return new Date(chaineDate).toLocaleString('fr-FR');
      };

      const basculerApiCle = (idApi: string) => {
        if (apiSelectionnee === idApi) {
          // If clicking on the same API, close it
          setApiSelectionnee(null);
          setAfficherClesApi(false);
        } else {
          // If clicking on a different API, close the current one and open the new one
          setApiSelectionnee(idApi);
          setAfficherClesApi(true);
        }
      };
      ...
      return (
        <div className="space-y-3 sm:space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              ...
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    value={rechercheApis}
                    onChange={(e) => {
                      setRechercheApis(e.target.value);
                    }}
                    className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                  />
                  {rechercheApis && (
                    <button
                      onClick={() => setRechercheApis('')}
              ...
              <div className="flex items-center space-x-2">
                ...
                <select
                  ...
                  onChange={(e) => setElementsParPage(parseInt(e.target.value))}
            ...
            {rechercheApis && (
              <div className="mt-3 pt-3 border-t border-gray-100">
                <p className="text-sm text-gray-600">
                  {apisFiltrees.length} résultat{apisFiltrees.length > 1 ? 's' : ''} trouvé{apisFiltrees.length > 1 ? 's' : ''} pour "{rechercheApis}"
                </p>
              </div>
            )}
          </div>
          ...
          <div className="block sm:hidden space-y-1">
            {apisActuelles.map((api) => (
              <div key={api.id} className="space-y-1">
                ...
                {apiSelectionnee === api.id && afficherClesApi && (
                      ...
                      {api.clesApi.map((apiKey) => (
                        <div key={apiKey.id} className="border border-gray-200 rounded-lg p-1.5">
                          ...
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <div className="flex items-center space-x-1">
                              <span className={`inline-flex px-1 py-0.5 text-xs font-semibold rounded-full ${obtenirCouleurStatut(apiKey.statut)}`}>
                                {apiKey.statut}
                              </span>
                              <button 
                                onClick={() => setAfficherCle({...afficherCle, [apiKey.id]: !afficherCle[apiKey.id]})}
                                className="text-gray-400 hover:text-gray-600 p-0.5 rounded hover:bg-gray-50"
                              >
                                {afficherCle[apiKey.id] ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="bg-gray-50 rounded p-1.5 mb-1.5">
                            <div className="flex flex-col space-y-1">
                              <code className="text-xs font-mono text-gray-700 break-all leading-relaxed">
                                {afficherCle[apiKey.id] ? apiKey.cle : '••••••••••••••••••••••••••••••••'}
                              </code>
                              <div className="flex items-center justify-end space-x-1">
                                <button
                                  onClick={() => copierDansPressePapiers(apiKey.cle, apiKey.id)}
                                  className="flex items-center space-x-1 text-blue-600 hover:text-blue-800 px-1 py-0.5 rounded text-xs hover:bg-blue-50"
                                >
                                  {cleCopiee === apiKey.id ? <CheckCircle className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                                  <span>{cleCopiee === apiKey.id ? 'Copié' : 'Copier'}</span>
                                </button>
                                <button
                                  onClick={() => regenererCle(api.id, apiKey.id)}
                          ...
                          <div className="mb-1.5">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs font-medium text-gray-900">{formaterDateHeure(apiKey.utilisation.dernierAcces)}</span>
                              </div>
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-gray-600">Expire le</span>
                                <span className="text-xs font-medium text-gray-900">{formaterDate(apiKey.dateExpiration)}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="border-t pt-1.5">
                            <button
                              onClick={() => setAfficherJournalUtilisation(afficherJournalUtilisation === apiKey.id ? null : apiKey.id)}
                          ...
                          {/* Usage Log Display */}
                          {afficherJournalUtilisation === apiKey.id && apiKey.usageLog && (
                                  ...
                                  <div key={index} className="bg-gray-50 rounded p-1.5 text-xs">
                                    <div className="space-y-0.5">
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className="text-gray-900">{formaterDateHeure(log.timestamp)}</span>
                                      </div>
                                      <div className="flex items-center justify-between">
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className={`inline-flex px-1 py-0.5 text-xs font-semibold rounded-full ${obtenirCouleurMethode(log.method)}`}>
                                          {log.method}
                                        </span>
                                      </div>
                                      <div className="flex items-center justify-between">
                                        <span className="text-gray-600 font-medium">Statut</span>
                                        <span className={`font-medium ${obtenirCouleurStatut(log.status)}`}>
                                          {log.status}
                                        </span>
                                      </div>
                                      <div className="flex items-center justify-between">
                      ...
                      {api.clesApi.flatMap(key => key.usageLog || []).slice(0, 5).map((log, index) => (
                        <div key={index} className="bg-gray-50 rounded p-1 text-xs">
                          <div className="flex items-center justify-between mb-0.5">
                            <span className="text-gray-600">{formaterDateHeure(log.timestamp)}</span>
                            <span className={`inline-flex px-1 py-0.5 text-xs font-semibold rounded-full ${obtenirCouleurMethode(log.method)}`}>
                              {log.method}
                            </span>
                          </div>
                        ...
                        <tr className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap">
                            <div className="flex items-center space-x-2">
                              ...
                              <button 
                                onClick={() => {
                                  setApiSelectionnee(apiSelectionnee === api.id ? null : api.id);
                                  setAfficherClesApi(!afficherClesApi);
                                }}
                                    ...
                                    <div key={apiKey.id} className="border border-gray-200 rounded-lg p-3">
                                      <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center space-x-2">
                                          ...
                                          <div className="min-w-0 flex-1">
                                            ...
                                            <p className="text-xs text-gray-500">Créée le {formaterDate(apiKey.dateCreation)}</p>
                                          </div>
                                        </div>
                                        <div className="flex items-center space-x-2">
                                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${obtenirCouleurStatut(apiKey.statut)}`}>
                                            {apiKey.statut}
                                          </span>
                                          <button 
                                            onClick={() => setAfficherCle({...afficherCle, [apiKey.id]: !afficherCle[apiKey.id]})}
                                          ...
                                          >
                                            {afficherCle[apiKey.id] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                          </button>
                                        </div>
                                      </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                          ...
                                          <div className="flex items-center space-x-2">
                                            <button
                                              ...
                                              title="Copier la clé"
                                            >
                                              {cleCopiee === apiKey.id ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                                            </button>
                                            <button
                                              onClick={() => regenererCle(api.id, apiKey.id)}
                                      ...
                                      <div className="mb-3">
                                        <div className="flex items-center justify-between mb-2">
                                          ...
                                          <button
                                            onClick={() => setModifierPermissions({...modifierPermissions, [apiKey.id]: !modifierPermissions[apiKey.id]})}
                                            className="text-blue-600 hover:text-blue-800 text-xs flex items-center space-x-1"
                                          >
                                            <Edit className="w-3 h-3" />
                                            <span>{modifierPermissions[apiKey.id] ? 'Annuler' : 'Modifier'}</span>
                                          </button>
                                        </div>
                                        
                                        {modifierPermissions[apiKey.id] ? (
                                          <div className="space-y-2">
                                            ...
                                            <div className="flex space-x-2">
                                              <button
                                                onClick={() => mettreAJourPermissions(api.id, apiKey.id, apiKey.permissions)}
                                                className="px-2 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700"
                                              >
                                                Sauvegarder
                                              </button>
                                              <button
                                                onClick={() => setModifierPermissions({...modifierPermissions, [apiKey.id]: false})}
                                      ...
                                      <div className="grid grid-cols-4 gap-3 text-xs mb-3">
                                        ...
                                        <div className="flex items-center space-x-1">
                                          ...
                                          <span className="text-gray-700 truncate">Dernier: {formaterDateHeure(apiKey.utilisation.dernierAcces)}</span>
                                        </div>
                                        <div className="flex items-center space-x-1">
                                          <Calendar className="w-3 h-3 text-gray-600 flex-shrink-0" />
                                          <span className="text-gray-700 truncate">Expire: {formaterDate(apiKey.dateExpiration)}</span>
                                        </div>
                                      </div>
                                                ...
                                                {apiKey.usageLog.map((log, index) => (
                                                  <tr key={index} className="hover:bg-gray-50">
                                                    <td className="px-2 py-1 text-gray-700 text-xs">
                                                      {formaterDateHeure(log.timestamp)}
                                                    </td>
                            ...
                            <td colSpan={7} className="px-4 py-3 bg-gray-50">
                              <div className="bg-white rounded-lg border border-gray-200 p-3">
                                <div className="flex items-center justify-between mb-3">
                                  ...
                                  <button 
                                    onClick={() => setAfficherLogsApi(null)}
                                    className="text-gray-400 hover:text-gray-600 p-1 rounded hover:bg-gray-50"
                                    title="Fermer"
                                  >
                                    <X className="w-4 h-4" />
           ...
           <div className="bg-gray-50 px-2 sm:px-3 py-2 flex items-center justify-between border-t border-gray-200">
                <div className="flex-1 flex justify-between sm:hidden">
                  ...
                  <button 
                    ...
                    disabled={pageActuelle === pagesTotales}
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-xs text-gray-700">
                      Affichage de <span className="font-semibold">{indexPremierElement + 1}</span> à <span className="font-semibold">{Math.min(indexDernierElement, apisFiltrees.length)}</span> sur <span className="font-semibold">{apisFiltrees.length}</span> résultat{apisFiltrees.length > 1 ? 's' : ''}
                    </p>
                  </div>
                  <div>
                    <nav className="relative z-0 inline-flex rounded-lg shadow-sm -space-x-px" aria-label="Pagination">
                      ...
                      {Array.from({ length: pagesTotales }, (_, i) => i + 1).map((page) => (
                        <button
                          key={page}
          ...
          {afficherModalSuppression && apiASupprimer && (
            <div className="fixed inset-0 bg-white/40 backdrop-blur-sm overflow-y-auto h-full w-full z-50">
              <div className="relative top-20 mx-auto p-4 border w-80 shadow-xl rounded-xl bg-white">
                <div className="mt-2 text-center">
                  ...
                  <div className="mt-2 px-6 py-2">
                    <p className="text-xs text-gray-500">
                      Êtes-vous sûr de vouloir supprimer l'API <strong>{apiASupprimer.nom}</strong> ?
                    </p>
                  ...
                  <div className="flex items-center justify-center space-x-2 mt-3">
                    ...
                    <button
                      onClick={confirmerSuppressionApi}
