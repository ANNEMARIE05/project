          ...
          <div className="block sm:hidden">
            {utilisateursCourants.length > 0 ? (
                  ...
                  <div key={user.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-3">
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium">{user.totalDocuments || 0}</span>
                      </div>
                      <div className="flex items-center justify-between">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
                ...
                <h3 className="text-sm font-medium text-gray-900 mb-1">Aucun utilisateur trouvé</h3>
                <p className="text-xs text-gray-500">
                  {recherche ? `Aucun utilisateur ne correspond à "${recherche}"` : 'Aucun utilisateur disponible'}
                </p>
                {recherche && (
                  <button
                    onClick={reinitialiserRecherche}
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {utilisateursCourants.map((user) => (
                      <tr key={user.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 whitespace-nowrap">
                          <div className="text-xs text-gray-900">{user.totalDocuments || 0}</div>
                        </td>
                        <td className="px-4 py-2 whitespace-nowrap">
                          {user.invitationSent ? (
                            <div className="flex items-center space-x-1">
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-xs text-gray-700">
                      Affichage de <span className="font-semibold">{indexPremierUtilisateur + 1}</span> à <span className="font-semibold">{Math.min(indexDernierUtilisateur, utilisateursFiltres.length)}</span> sur <span className="font-semibold">{utilisateursFiltres.length}</span> résultats
                    </p>
                  </div>
                  <div>
                    <nav className="relative z-0 inline-flex rounded-lg shadow-sm -space-x-px" aria-label="Pagination">
                      ...
                      {Array.from({ length: Math.min(3, nombreTotalPages) }, (_, i) => {
                        const pageNum = pageCourante - 1 + i;
                        if (pageNum < 1 || pageNum > nombreTotalPages) return null;
                        return (
                          <button 
                            key={pageNum}
                            onClick={() => allerPage(pageNum)}
                            className={`relative inline-flex items-center px-2.5 py-1 border border-gray-300 text-xs font-medium transition-colors ${
                              pageNum === pageCourante 
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <label className="text-xs font-medium text-gray-700">Total documents</label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <span className="text-lg font-bold text-green-600">{utilisateurSelectionne.totalDocuments || 0}</span>
                          <span className="text-xs text-gray-500">documents</span>
                        </div>
                      </div>
